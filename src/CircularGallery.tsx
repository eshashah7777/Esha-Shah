...
class Title {
  ...
  onResize() {
    if (!this.mesh) return;
    const planeAspect = this.plane.scale.y > 0 ? this.plane.scale.x / this.plane.scale.y : 1.6;
    const textHeight = 0.16;
    const textWidth = (textHeight * this.textAspect) / planeAspect;
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          float planeAspect = (uPlaneSizes.y > 0.0) ? (uPlaneSizes.x / uPlaneSizes.y) : 1.55;
          float imageAspect = (uImageSizes.y > 0.0) ? (uImageSizes.x / uImageSizes.y) : 1.0;
          
          // Fit entire logo within 72% of card dimensions with generous breathing margin so nothing is cropped
          float logoScale = 0.72;
          
          vec2 scale;
          if (imageAspect > planeAspect) {
            // Image is wider than card
            scale.x = 1.0 / logoScale;
            scale.y = scale.x * (planeAspect / imageAspect);
          } else {
            // Image is taller or square
            scale.y = 1.0 / logoScale;
            scale.x = scale.y * (imageAspect / planeAspect);
          }
          ...
          // Crisp clean white card backing so the entire logo is clearly visible with pure contrast
          vec3 cardBg = vec3(1.0, 1.0, 1.0);
          vec3 blendedRgb = mix(cardBg, color.rgb, color.a);
          
          gl_FragColor = vec4(blendedRgb, alpha);
        }
      `,
      uniforms: {
        tMap: { value: texture },
        uPlaneSizes: { value: [0, 0] },
        uImageSizes: { value: [600, 600] },
        uSpeed: { value: 0 },
    ...
    img.onerror = () => {
      console.warn(`CircularGallery: Failed to load image from ${this.image}`);
    };
  }
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    // Landscape format & smaller card size for balanced visibility of the whole logo
    this.plane.scale.y = (this.viewport.height * (300 * this.scale)) / this.screen.height;
...
export const DEFAULT_BRAND_ITEMS = [
  {
...
class App {
  ...
  createMedias(
    ...
    const galleryItems = items && items.length ? items : DEFAULT_BRAND_ITEMS;
